...
class CallbackHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        # wait for 1 second to ensure the authorization code is received
        time.sleep(1)
        self.wfile.write(b'''
            <html>
            <body>
...
def get_prompt_info_from_playlist(playlist_href, access_token):
    ...
    for track in all_tracks:
        curr_obj = {}
        curr_obj['name'] = track['name']
        curr_obj['artist'] = track['artists'][0]['name']
        curr_obj['id'] = track['id']
        if curr_obj['artist'] not in artist_map:
            artist_map[curr_obj['artist']] = get_artist(track['artists'][0]['id'], access_token)
            curr_obj['genres'] = artist_map[curr_obj['artist']]['genres']
            
            
        else:
            curr_obj['genres'] = artist_map[curr_obj['artist']]['genres']
        print(f'Artist: {curr_obj["artist"]},  Genres: {curr_obj["genres"]}')
